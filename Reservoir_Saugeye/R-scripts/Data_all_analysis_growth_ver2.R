...
LMM1_Best <- lmer(growth ~ sto_doy + (1|location), REML = FALSE)
r2_nakagawa(LMM1_Best, by_group = FALSE)

LMM2_Best <- lmer(growth ~ sto_doy + sto_dens_ha  + (1|location), REML = FALSE)
r2_nakagawa(LMM2_Best, by_group = FALSE)

LMM3_Best <- lmer(growth ~ sto_doy + lsaug_cpue + (1|location), REML = FALSE)
r2_nakagawa(LMM3_Best, by_group = FALSE)

LMM4_Best <- lmer(growth ~ sto_doy + lcrap_cpue + (1|location), REML = FALSE)
r2_nakagawa(LMM4_Best, by_group = FALSE)

LMM5_Best <- lmer(growth ~ sto_doy + secchi + (1|location), REML = FALSE)
